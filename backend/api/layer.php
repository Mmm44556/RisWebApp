    ...
    if (count($arr) !== 0) {
        ...
        foreach ($arr as $key => $files) {
            ...
            if (filetype($files) === 'dir') {
                ...
                $fileFolder = str_replace('/', '', $folder);
                array_push($a, array('id'=>$key,'text' => $folder, 'type' => $type, 'timeStamp' => $timeStamp, 'selected' => false, 'filename' => $fileFolder));
                ...
                array_push($a, array('id'=>$key,'text' => $file, 'selected' => false, 'type' => $fileType, 'timeStamp' => $fileTimeStamp, 'size' => round($fileSize, 3),'state' => array(), 'filename' => $file));
