...
if (count($files) !== 0) { //查詢檔案路徑
    foreach ($files as $key => $path) {
        ...
        $type = filetype($path);
        $p = (count($final) - 1);
        array_push($ar, array('id'=>$key,'filename' => strval($final[$p]),'text' => strval($final[$p]), 'selected' => false, 'type' => $type, 'timeStamp' => $timeStamp, ));
    ...
    echo json_encode([array('type' => 0, 'selected' => false)]);
};